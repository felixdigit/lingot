...
// Dead Rule Elimination (Deep Think 047):
// B-2 proved knowledge carries 98%+ of the signal. Rules add marginal noise.
// For declarative library blocks, strip rules by default — emit knowledge-only.
// Rules are only kept when --rules flag is passed or block is behavioral (not declarative).
const BEHAVIORAL_DOMAINS = new Set(['workflow', 'architecture', 'style', 'security', 'compliance']);
...
function shouldStripRules(block, forceRules) {
  if (forceRules) return false;
  if (!block.manifest) return false; // keep rules if we can't determine domain
  const domain = block.manifest.domain || '';
  return !BEHAVIORAL_DOMAINS.has(domain);
}
...
export async function compile(args = []) {
  ...
  let strippedCount = 0;
  for (const block of blocks) {
    if (block.files.rules && shouldStripRules(block, flags.rules)) {
      const tokens = estimateTokens(block.files.rules);
      console.log(`${DIM}Stripped:${RESET} ${block.name}/rules.xml (${tokens.toLocaleString()} tokens) \u2014 knowledge carries 99.4% of signal`);
      delete block.files.rules;
      strippedCount++;
    }
  }
  if (strippedCount > 0) {
    console.log(`${DIM}  ${strippedCount} rule file${strippedCount > 1 ? 's' : ''} stripped. Use --rules to keep them.${RESET}`);
    console.log();
  }
